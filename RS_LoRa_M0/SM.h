...
void StationMonitor() {
  ...
  if (ds_found[0] || ds_found[1]) {
    ...
    sprintf (Buffer32Bytes, "No Soil Moisture");
  }
  len = (strlen (Buffer32Bytes) > 21) ? 21 : strlen (Buffer32Bytes);
  ...
  // Line 3 of OLED
  // =================================================================
  sprintf (Buffer32Bytes, "RG:%3d %d.%02d %04X", 
    (int) rainguage_interrupt_count,    // Pins are 12bit resolution (0-4095), values need to be (0-1023)
